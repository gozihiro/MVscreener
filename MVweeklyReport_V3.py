...
def create_intelligence_report(df):
    ...
    for _, row in stock_rows.iterrows():
        ...
        stocks_json.append({
            "ticker": str(row['銘柄']), "prices": prices, "patterns": patterns,
            "growths": growths, "launchpads": launchpads
        })

    full_data_payload = {"dates": [f"2026/{d}" for d in dates], "market": market_data, "stocks": stocks_json}
    ...
    <head>
        <meta charset="UTF-8">
        <title>Strategy Intelligence V3</title>
        <script src="https://cdn.plot.ly/plotly-2.24.1.min.js"></script>
        <style>
            body {{ font-family: 'Segoe UI', sans-serif; background: #f0f4f8; margin: 0; padding: 20px; color: #2c3e50; }}
            ...
            .date-input {{ background: #2c3e50; border: 1px solid #455a64; color: white; padding: 10px; border-radius: 8px; cursor: pointer; }}
            ...
            .persistence-tag {{ float: right; background: #e74c3c; color: white; padding: 4px 10px; border-radius: 6px; font-size: 0.8em; }}
        ...
        <script>
            ...
            function updateDashboard(targetDates) {{
                ...
                const analyzed = data.stocks.map(s => {{
                    ...
                    // 【重要修正】最新から遡り、最初の有効なパターンを採用。breakで上書きを防止。
                ...
                const sections = [
                    ...
                    {{ title: "🏆 総合・サバイバルリーダー", hint: "定着 ➔ 騰落率 ➔ 発射台 ➔ 成長 ➔ 低ボラ", 
                        data: [...analyzed].sort(getSorter(['persistence','change','launchpad','growth','vol'], [-1,-1,-1,-1,1])).slice(0,5), isTotal: true }},
                    ...
                    {{ title: "🌀 VCP・収束リーダー", hint: "定着 ➔ 発射台 ➔ 低ボラ ➔ 成長 ➔ 騰落率", 
                        data: analyzed.filter(x => x.pattern.includes('VCP')).sort(getSorter(['persistence','launchpad','vol','growth','change'], [-1,-1,1,-1,-1])).slice(0,5) }},
                    {{ title: "⚡ PowerPlay・勢いリーダー", hint: "定着 ➔ 騰落率 ➔ 発射台 ➔ 成長 ➔ 低ボラ", 
                        data: analyzed.filter(x => x.pattern.includes('PowerPlay')).sort(getSorter(['persistence','change','launchpad','growth','vol'], [-1,-1,-1,-1,1])).slice(0,5) }}
                ];
                    ...
                    sec.data.forEach((s, idx) => {{
                        ...
                        <div class="rank-card">
                            ...
                            <div class="metric-box">
                                <div class="metric-row"><span>発射台Score</span> <b style="color:#f39c12">${{s.launchpad}}</b></div>
                ...
                Plotly.newPlot('chart-scatter', [{{
                    ...
                    marker: {{ size: 14, color: analyzed.map(x => x.launchpad), colorscale: 'YlOrRd', showscale: true, colorbar: {{title: 'Score'}} }}
